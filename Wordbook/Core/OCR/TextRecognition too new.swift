...
class TextRecognition {
    ...
    func naturalLanguageProcessing(meaningCandidates: [[VNRecognizedTextObservation]]) {
        ...
        translateArray(meaningCandidates: meaningCandidates, allTranslateCompletion: { results in
            meaningCandidates_english = results
            semaphore.signal()
        })
        semaphore.wait()
        print(meaningCandidates_english[0][0])
    ...
    func translateArray(meaningCandidates: [[VNRecognizedTextObservation]], allTranslateCompletion: @escaping ([[String]]) -> Void){
    ...
    translatorForDownloading.downloadModelIfNeeded { error in
        ...
        if translatorForDownloading == self.translator {
            ...
            for i in 0..<meaningCandidates.count {
                ...
                for j in 0..<meaningCandidates[i].count {
                    ...
                    let semaphore = DispatchSemaphore(value: 0)
                    translate(text: meaning ,completion: {result in
                        meaningCandidates_english_i.append(result)
                        semaphore.signal()
                    }
                    )
                    semaphore.wait()
        ...
        allTranslateCompletion(meaningCandidates_english)
    }
}
    }
